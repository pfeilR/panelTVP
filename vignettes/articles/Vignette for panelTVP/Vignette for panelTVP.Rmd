...
When summarizing the results in a table, the user can order the results either by covariate
...
In this section we describe how zero-inflated and overdispersed count data can be analyzed with the \texttt{panelTVP()} function. We consider synthetic data that we simulate using the function \texttt{sim\_panelTVP()}. As missing values in the response variable are supported and imputed in a data augmentation step by \texttt{panelTVP()}, we will manually set values of the response variable at missing. Finally, we compare our shrinkage prior approach to an unstructured approach, where independent Normal priors are placed on the regression effects and the factor loadings.
...
The regression effects and the factor loadings are then simulated from the first-order random walks that were already discussed in the theoretical chapter of this vignette.
...
shrink.zinb <- panelTVP(y ~ W1.nb + W2.nb | W1.logit + W2.logit,
                        ...
                        model = "ZINB",
                        mcmc.opt = list(chain.length = 1000, burnin = 500, thin = 10, asis = T),
                        progress.bar = T)
